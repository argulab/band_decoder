...
char RelayPins [] = {32, 34, 36, 38, 40, 42, 44, 46};

//------------------------------------- DEBUG
void debug_relay_states () {
  for (uint8_t i = 0; i <= MaxRelay; i++) {
    uint8_t pinRelay = RelayPins [i];
    Serial.print ("Rele num = ");
...
uint8_t select_relay_antenna (antennas ant) {
  switch (ant) {
    case hexbeam:   return 0;
    case aperiodic: return 1;
    case ant3:      return 2;
    case ant4:      return 3;
  }
}
...
void service_switch_antenna (antennas antennaService) {
  uint8_t activeRelay = select_relay_antenna (antennaService);
  
  
  for (uint8_t relayNum = 0; relayNum <= MaxRelay; relayNum++) {
    uint8_t pinRelay = RelayPins [relayNum];
    if (relayNum == activeRelay) {
      digitalWrite (pinRelay, HIGH);
    } else {
      digitalWrite (pinRelay, LOW);
    }
  }
  #ifdef DEBUG_RELAYS_STATES
    debug_relay_states ();
  #endif  //DEBUG_RELAYS_STATES
}
